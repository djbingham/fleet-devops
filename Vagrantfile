...
# downloaded on 25th February 2016. Docker-Compose is installed to /opt/bin for easy container linking.
# ------------------------------------------------- * -------------------------------------------------
#
	...
	(1..$num_instances).each do |i|
		config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
			...
			if File.file?(CUSTOM_PROVISION_PATH)
				config.vm.provision :shell, :run => "always", :path => CUSTOM_PROVISION_PATH
			end
		end
	end
end
